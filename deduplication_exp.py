...
def create_subplot(ax, zonghe_data, total_pareto_data, array_size, net_name):
    ...
    # Draw bar chart - only two bars now
    bars1 = ax.bar(x - width/2, before_pruning, width, label='Before Deduplication', color='#5DA5DA', alpha=0.8)
    bars2 = ax.bar(x + width/2, after_pruning, width, label='After Deduplication', color='#FAA43A', alpha=0.8)
    ...
    def add_value_labels(bars):
        for bar in bars:
            ...
            if height > 0:  # Only show labels for values > 0
                ax.text(bar.get_x() + bar.get_width()/2., height + max(before_pruning + after_pruning)*0.01,
...
def run_network_experiments(net_name, array_configs, output_dir='duplicate'):
    ...
    for array_row, array_col in selected_arrays:
        ...
        try:
            if net_name == 'Resnet20' or net_name == 'vgg16':
                ...
                fangan, zonghe, total_pareto = run_experiment_for_network_and_array(net_name, array_row, array_col)
                all_data[f"{array_row}x{array_col}"] = (zonghe[0:20], total_pareto[0:20])
...
def main():
    ...
    networks = ['vgg16', 'Resnet20','Resnet110','sqtf']
