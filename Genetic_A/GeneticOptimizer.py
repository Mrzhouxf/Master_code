...
def allocate_chips(mapping_stratepy: List[List[Any]], num_tile: int, num_chiplet: int) -> List[Dict[str, Union[int, float, bool]]]:
    """
...
def allocate_chips_new(mapping_stratepy: List[List[Any]], num_tile: int, num_chiplet: int) -> List[Dict[str, Union[int, float, bool]]]:
    ...
    修正了起始层映射状态判断的逻辑：
    - 拆分到新芯片的起始层(start_layer)始终标记为 start_layer_complete=False。
    ...
    # 额外追踪变量：记录被部分分配的层，它在当前芯片上已经消耗了多少资源
    # (用于准确计算下一个芯片的 start_res)
    layer_resources_used_so_far = {} 
    
    while current_layer_idx < total_layers and current_chip_id < num_chiplet:
        layer_1based = current_layer_idx + 1
        
        # 1.确定当前层资源需求(resource_required)和该层在当前芯片的起始资源(layer_res_to_use)
        layer_res_total = int(mapping_stratepy[current_layer_idx][2]) # 该层总共需要的资源
        
        # 如果当前层是延续层(即它已经被前面的芯片部分分配过)
        if layer_1based in layer_resources_used_so_far:
            res_allocated_on_prev_chips = layer_resources_used_so_far[layer_1based]
            layer_res_to_use = layer_res_total - res_allocated_on_prev_chips
        else:
            # 全新层
            layer_res_to_use = layer_res_total
            
        # 2.确定当前芯片的Start Layer属性(只在芯片首次分配时执行)
        if len(chips) <= current_chip_id:
            # 确定 start_layer_complete 状态
            # 如果起始层与上一个芯片的结束层相同，则它一定是延续，start_complete=False
            if prev_chip and prev_chip['end_layer'] == current_start_layer and not prev_chip['end_layer_complete']:
                start_complete = False
                # 起始资源应该是该层在当前芯片上实际需要使用的资源(即layer_res_to_use)
                start_res = layer_res_to_use 
            else:
                # 否则，它是一个新的、未被拆分的层的开始，start_complete=True
                start_complete = True
                start_res = layer_res_total # 此时layer_res_to_use == layer_res_total
            ...
            chip_record = {
                ...
                'start_layer': current_start_layer,
                'start_layer_complete': start_complete,
                'start_layer_resources': start_res, # 初始值，可能会在分配过程中被layer_res_to_use覆盖
                'end_layer': 0,
                'end_layer_complete': False,
                'end_layer_resources': 0
            }
        ...
        # 实际分配的资源量：取'层所需资源'和'芯片剩余资源'中的最小值
        allocated_res_on_current_chip = min(layer_res_to_use, current_remaining_tile)
        ...
        # 更新该层总共被使用的资源(用于下一个芯片的计算)
        layer_resources_used_so_far[layer_1based] = layer_resources_used_so_far.get(layer_1based, 0) + allocated_res_on_current_chip
...
def generate_traces_nop_GA(bus_width, netname, nop_records, scale):
    """
    为片上网络(NoC)生成通信 trace 文件
    每个 Chiplet 每层生成一个 txt,记录 (src, dest, timestamp) 三列
    """
