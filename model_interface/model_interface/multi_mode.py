...
def _dlbp_opt_flexible_worker(worker_input: Tuple) -> Optional[OptimizationResult]:
    """
    灵活优化模式的工作进程：支持 H, L, U 模式，使用 DLBP 和动态上界剪枝。
    """
    (
        start_scheme,
    ...
    # 局部最优解追踪
    # 初始化最优值：对于 H/L/U，越大越好；对于 R，越小越好。
    # 对于最小化问题，初始值设为无穷大
    ...
    # 定义目标函数和剪枝逻辑
    primary_idx = -1 # 0: Transmission, 1: Cycles, 2: Resource
    secondary_idx = -1 
    
    if optimization_target == 'H' or optimization_target == 'C': # Cycles -> Transmission
        primary_idx, secondary_idx = CYCLES_IDX, TRANSMISSION_IDX
    elif optimization_target == 'L': # Transmission -> Cycles
        primary_idx, secondary_idx = TRANSMISSION_IDX, CYCLES_IDX
    elif optimization_target == 'R': # Resource
        primary_idx, secondary_idx = RESOURCE_IDX, -1 # Secondary is not strictly used for comparison in R
    elif optimization_target == 'U':
        # U mode doesn't have a simple primary/secondary index, we use the utility value.
        pass
    else:
        print(f"警告: 未知优化目标 {optimization_target}")
        return None
    ...
    def check_and_update_best(
        ...
        if current_value < best_value:
            ...
            # 只有 H, L, R 模式需要次要目标/资源平局判断
            if optimization_target == 'H' or optimization_target == 'C':
                # H/C: Cycles 相同，比较 Transmission (次要目标)
                if current_transmission < best_totals[TRANSMISSION_IDX]:
                    is_better = True
            elif optimization_target == 'L':
                # L: Transmission 相同，比较 Cycles (次要目标)
                if current_cycles < best_totals[CYCLES_IDX]:
                    is_better = True
            elif optimization_target == 'R':
                 # R: Resource 相同，不需要进一步比较，不更新
                 pass
    ...
    def check_pruning(
        ...
        """检查 DLBP 和动态上界剪枝是否允许继续搜索"""
        
        # 1. 动态上界剪枝 (核心优化)
        # 如果当前累积值已经大于等于当前找到的最佳值，则剪枝
        if current_value >= best_value:
            return True # 剪枝
        ...
        # 3. 目标下限剪枝 (Suffix Sum Pruning) - 适用于 H, L, U 模式
        if layer_index < total_layers and optimization_target != 'R':
            remaining_min_metric = 0.0
            if optimization_target == 'H' or optimization_target == 'C':
                remaining_min_metric = min_cycles_suffix_sum[layer_index]
            elif optimization_target == 'L':
                remaining_min_metric = min_transmission_suffix_sum[layer_index]
            elif optimization_target == 'U':
                # U 模式的下限剪枝很难准确计算，因为涉及到归一化。
                # 简单近似：如果两个指标的最小总和都无法超过当前最佳值，则剪枝
                pass # 暂不实现 U 模式的下限剪枝，依赖动态上界剪枝和 DLBP。

            if current_value + remaining_min_metric >= best_value:
                return True # 剪枝
    ...
    def search_scheme(
        ...
        # 2. 动态/下限剪枝 (在递归进入前检查)
        # 注意: 只有在非终止层，我们才需要检查 Lower Bound Suffix Sums
        if layer_index < total_layers:
             # 计算当前路径的指标值
            current_opt_value = float('inf')
            if optimization_target == 'U':
                # U mode uses utility value for dynamic pruning, but requires normalization. 
                # We can only perform effective pruning when the final combo is calculated.
                # For ResNet110, we must rely primarily on DLBP and the final check.
                pass # Skipping mid-path dynamic pruning for U to simplify logic
            elif optimization_target == 'R':
                current_opt_value = current_resource
                
            if check_pruning(current_opt_value, layer_index, current_resource):
                return
    ...
    if optimization_target == 'U':
         ...
         best_value = cycle_limit + transmission_limit + 1.0 # 足够大的初始值
...
def _prepare_and_dispatch_optimizer(
    ...
    # 步骤 1: 单层预筛选 (只需要满足 Resource 上限即可)
    valid_schemes_per_layer = []
    max_cycles_global = 0.0 # 用于 U 模式归一化
    max_transmission_global = 0.0 # 用于 U 模式归一化
    ...
    for res in valid_results:
        ...
        if current_value < best_value:
            ...
            best_result = res
             ...
             # 平局处理：H/C 模式比较 Transmission，L 模式比较 Cycles
    ...
    if optimization_target == 'U':
         ...
         print(f"最优 Cycles: {best_result[CYCLES_IDX]:.4f}")
    elif optimization_target == 'L':
         print(f"全局最优方案已写入 {output_file} 文件中。目标: 最小 Transmission (次要目标: 最小 Cycles)。")
         print(f"最优 Transmission: {best_result[TRANSMISSION_IDX]:.4f}")
    elif optimization_target == 'R':
         print(f"全局最优方案已写入 {output_file} 文件中。目标: 最小 Resource。")
         print(f"最优 Resource: {best_result[RESOURCE_IDX]:.4f}")
...
def process_parallel_opt_U(
    ...
    需要提供权重 (W_T, W_C)，W_T 对应 Transmission，W_C 对应 Cycles。
    """
    if sum(weights) == 0:
